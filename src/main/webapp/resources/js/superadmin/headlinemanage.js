...
function listHeadTitleManagementInfo() {
    ...
    $.ajax({
        url : "http://localhost:8081/admin/getHeadLine",
        type : "post",
        dataType : 'json',
        data : {
        ...
        success : function(data) {
            var headlineDate = $.parseJSON(data.headLines);  // change the JSON string to javascript object
            $('#headTitleManagementTable').datagrid('loadData', headlineDate);
        }
    });
}
...
function ajaxTable() {
    ...
    var enableStatusBuffer = "";
    enableStatusBuffer += '<select id="headTitleManagementFilter_enableStatus" class="easyui-combobox" style="margin :2px; padding :4px;" ';
    enableStatusBuffer += 'onchange="changeFilterStatus(this.options[this.options.selectedIndex].value)">';
    enableStatusBuffer += '<option id="headTitleManagementFilter_ALL" value="">全部</option>';
    enableStatusBuffer += '<option id="headTitleManagementFilter_YES" value="1">启用</option>';
    enableStatusBuffer += '<option id="headTitleManagementFilter_NO" value="0">禁用</option></select>';
